...
def delete_add_user():
    """添加用户前，先删除数据，用例执行之后，再次删除以清理数据"""
    del_sql = base_data["init_sql"]["delete_add_user"]
    db.execute_db(del_sql)
    step_first()
    logger.info("添加用户操作：清理用户--准备添加新用户")
    logger.info("执行前置SQL：{}".format(del_sql))
    yield
    db.execute_db(del_sql)
    step_last()
    logger.info("添加用户操作：删除添加的用户")
    logger.info("执行后置SQL：{}".format(del_sql))

@pytest.fixture(scope="function")
def delete_get_user_info():
    """删除添加的获取用户数据用于再次添加验证查看用户数据接口是否正确"""
    del_sql = base_data["init_sql"]["delete_get_user_info"]
    db.execute_db(del_sql)
    step_first()
    logger.info("添加用户操作：清理用户--用于新增用户")
    logger.info("执行前置SQL：{}".format(del_sql))
