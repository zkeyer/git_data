...
def new_login_user(username,password,uuid):
    """
    登陆用户
    :param username: 用户名
    :param password: 密码
    ...
    result.msg = res.json()["msg"]
    result.response = res
    logger.info("登录用户 ==>> 返回结果 ==>> {}".format(result.get_response_json()))
    return result

def add_user(add_usernick,add_username,add_password,res_token):
    """
        添加用户
        :param add_usernick: 用户昵称
        :param password: 密码
        :param add_username: 用户名称
        :param res_token: token
        :return:
    """
    result = ResultBase()
    payload = {
        "nickName": add_usernick,
        "userName": add_username,
        "password": add_password
    }
    header =  {
        "authorization":"Bearer {}".format(res_token),
        "Content-Type": "application/json;charset=UTF-8",
        "Cookie":  "sidebarStatus=1; Admin-Token={}".format(res_token)
    }
...
def get_user_info(res_token):
    ...
    res = user.get_user_info(headers=header)
    if res == 200:
        result == True
    else:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()["code"], res.json()["msg"])
